...
class PayrollReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        for used_struct in used_structures:
            ...
            for rule in lines.slip_ids:
                for struct in rule.struct_id:
                    for arule in struct.rule_ids:
                        if arule.name not in rules:
                            rules.append(arule.name)
            ...
            columns = 2
            for rule in rules:
                 sheet.write(2,columns, rule,format1)
                 columns+=1
            ...
            for slip in lines.slip_ids:
                ...
                for line in slip.line_ids:
                    index = rules.index(line.name)
                    sheet.write(e_name, index+2, line.amount, format3)
                    columns+=1
                e_name += 1
