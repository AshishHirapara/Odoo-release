...
class PurchaseOrder(models.Model):
    ...
    purchase_request_id = fields.Many2one('sprogroup.purchase.request', required=True, index=True)

    @api.onchange('order_line')
    def onchange_total_amount(self):
        # account_ids = self.env.context.get('active_ids', [])
        acc = self.env['sprogroup.purchase.request'].browse(self.purchase_request_id.id)
    ...
    def button_confirm(self):
        request = self.env['sprogroup.purchase.request'].search([('id', '=', self.request_id.id)])
        if request.state != 'done' and request:
            raise ValidationError(_("The purchase request is not Done"))
        for request_line in request.line_ids:
            for order_line in self.order_line:
                if order_line.selection != "selected" and self.requisition_id:
                    raise ValidationError(_(order_line.product_id.name + "  is not selected"))
                if order_line.product_id == request_line.product_id and request_line.invoice_flag == False and request_line.purchased_qty != 0:
                    if (request_line.product_qty - request_line.purchased_qty) - order_line.product_qty < 0:
                        if not self.requisition_id.is_merge:
                            raise ValidationError(_("You can not increase quantity of " + str(
                                order_line.product_id.name) + ". The requsted quantity was " + str(
                                request_line.product_qty - request_line.purchased_qty) + " not " + str(
                    ...
                    if (request_line.product_qty - request_line.purchased_qty) - order_line.product_qty < 0:
                        if not self.requisition_id.is_merge:
                            raise ValidationError(
                                _("you can not increase quantity of " + str(order_line.product_id.name) +
                                  ". The requsted quantity was " + str(
                                    request_line.product_qty - request_line.purchased_qty) + " not " + str(
                                    order_line.product_qty)))
                            print("a")
                    request_line.write({
...
class SprogroupPurchaseRequest(models.Model):
    ...
    def button_draft(self):
        if self.line_ids.po_number_2:
            raise ValidationError(_("you can not reset the request once the order is confirmed"))
        ...
        for purchase_request_line in purchase_request_lines_ary:
            ...
            if approval_limits:
                ...
                if approver_user_id == self.env.user.id:
                    ...
                    raise ValidationError(_("The user doesnt Approval Limit"))
        ...
        if order_line:
            return {
                ...
                'context': {
                    ...
                    'default_requested_by': self.requested_by.id,
                    'default_approver_id': self.requested_by.id,
...
class SprogroupPurchaseRequestLine(models.Model):
    ...
    po_number_2 = fields.Many2one('purchase.order', string='Po Number')
