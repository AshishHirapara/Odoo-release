...
class PurchaseOrderLine(models.Model):
    ...
    def create(self, vals):
            ...
            if pur.requisition_id.id == account_ids[0]:
                values = {}
                values['product_id'] = vals[0].get('product_id')
                values['product_qty'] = vals[0].get('product_qty')
                values['price_unit'] = vals[0].get('price_unit')
                values['name'] = vals[0].get('name')
                values['partner_id'] = pur.partner_id.id
                values['price_subtotal'] = vals[0].get('price_subtotal')
                values['price_total'] = vals[0].get('price_total')
                values['taxes_id'] =  vals[0].get('taxes_id')
