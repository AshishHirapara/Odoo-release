...
class SaleOrder(models.Model):
    ...
    #     print("start compute order")
    #     taxes_grouped = self.get_taxes_values()
    #     print("tax group====", taxes_grouped)
    #     tax_lines = self.sale_tax_line_ids.browse([])
    #     for tax in taxes_grouped.values():
    #         tax_lines += tax_lines.new(tax)
    #     print("tax tax_lines====", tax_lines)
    #     return tax_lines

    sale_tax_line_ids = fields.One2many('sale.order.tax', 'sale_id', string='Tax Lines',
                                         states={'draft': [('readonly', False)]},
                                        store=True)
    # date_onchange = fields.Datetime(string="Date Time", default=fields.Datetime.now, required=False, readonly=True,
    #                                 select=True)

    # tax_amount = fields.Float(string="tax_amount")
    # tax_name = fields.Char(string='tax name')
    #
    # def init(self):
    #     tools.drop_view_if_exists(self._cr, 'customer_sales')
    #     self._cr.execute("""
    #           CREATE OR REPLACE VIEW customer_sales AS (
    #               SELECT  row_number() OVER () AS id, line.name As tax_name,line.amount AS tax_amount FROM(
    #               SELECT sot.name,sot.amount FROM sale_order_tax sot left join sale_order s  on
    #               (sot.sale_id= s.id))line
    #               )""")
