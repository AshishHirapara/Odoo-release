...
class HrPayslipRun(models.Model):
    ...
    def post_to_journal(self):
        ...
        if self.bank_account.id:
         credit_account_id = self.bank_account.id
        else:
            raise ValidationError(_(
                'You have not select bank account for the payroll!'))
        ...
        for slip in self.summary_id:
            ...
            # try:
            if slip.salary_detail.account_debit.id:
                    debit_account_id = slip.salary_detail.account_debit.id
                    amount = slip.amount

                    debit_line = (0, 0, {
                            'name':  slip.salary_detail.name,
                    ...
                    creadit_total += abs(amount)
            else:
                    raise ValidationError(_(
                        'You have not select debit account for the ' + slip.salary_detail.name))
            # except:
            #      raise ValidationError(_(
            #          'Please install payroll account'))
